    ...
    <section className="space-y-6 pb-10">
      ...
      <div className="grid gap-5 xl:grid-cols-[1fr_1fr]">
        <div className="rounded-[2rem] border border-white/10 bg-white/5 p-6">
          <p className="text-xs uppercase tracking-[0.28em] text-white/45">Lead capture finale</p>
          {leadStatus !== 'success' ? (
            <form className="mt-4 space-y-4" onSubmit={onLeadSubmit}>
              <div>
                <label className="mb-2 block text-sm font-medium text-white" htmlFor="email">
                  Vuoi ricevere il tuo report completo?
                </label>
                <p className="mb-3 text-sm leading-6 text-slate-300">
                  Ti inviamo il recap della run, il kit della tua classe e contenuti tecnici coerenti con il tuo archetipo.
                </p>
                <input
                  id="email"
                  type="email"
                  ...
                  onChange={(event) => onLeadChange({ ...leadForm, email: event.target.value })}
                />
              </div>
              <div className="grid gap-4 sm:grid-cols-2">
                <input
                  ...
                  value={leadForm.name}
                  onChange={(event) => onLeadChange({ ...leadForm, name: event.target.value })}
                />
                <select
                  className="field"
                  value={leadForm.interest}
                  onChange={(event) => onLeadChange({ ...leadForm, interest: event.target.value })}
                >
                  <option value="">Mi interessa soprattutto</option>
                  <option value="frontend">Frontend</option>
                  <option value="backend">Backend</option>
                  <option value="devops">DevOps</option>
                  <option value="ai">AI</option>
                  <option value="exploring">Sto esplorando</option>
                </select>
              </div>
              <label className="flex items-start gap-3 rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-slate-300">
                <input
                  ...
                  onChange={(event) => onLeadChange({ ...leadForm, consent: event.target.checked })}
                />
                <span>Acconsento a ricevere contenuti utili e aggiornamenti coerenti con il mio risultato.</span>
              </label>
              {leadStatus === 'error' ? (
                <p className="rounded-2xl border border-rose-400/25 bg-rose-500/10 px-4 py-3 text-sm text-rose-100">
                  Il portale CRM non ha risposto. Riprova tra poco oppure continua senza invio.
                </p>
              ) : null}
              <div className="flex flex-wrap gap-3">
                <button className="primary-button" type="submit" disabled={leadStatus === 'submitting'}>
                  {leadStatus === 'submitting' ? 'Sto inviando il report...' : 'Inviami il report'}
                </button>
                <button className="secondary-button" type="button" onClick={onSkipLead}>
                  Salta per ora
                </button>
              </div>
            </form>
          ) : (
            <div className="mt-4 rounded-[1.5rem] border border-emerald-400/20 bg-emerald-500/10 p-5 text-slate-100">
              <h3 className="text-xl font-semibold">Perfetto. Il tuo report e in viaggio.</h3>
              <p className="mt-3 text-sm leading-6 text-emerald-50/85">
                Nel frattempo puoi condividere il risultato, ritentare il raid o esplorare un altra classe.
              </p>
            </div>
          )}
        </div>
